...
type AuroraBanner() =
    ...
    member props.__ =
        div(class' = Lib.cn [|
            "z-40 flex flex-row items-center justify-center px-4 text-center text-sm font-medium bg-fd-background"
            props.class'
        |]).style'([
            Style.height "3rem"
        ]) {
    ...
    60deg, var(--end), var(--start) 2%, var(--start) 5%, transparent 8%, \
    transparent 14%, var(--via) 18%, var(--via) 22%, var(--mid) 28%, \
    var(--mid) 30%, var(--via) 34%, var(--via) 36%, transparent, \
    ...
    var(--end), var(--start) 4%, var(--start) 8%, transparent 9%, \
    transparent 14%, var(--mid) 16%, var(--mid) 20%, transparent, \
    var(--via) 36%, var(--via) 40%, transparent 42%, var(--end) 46%, \
    var(--end) calc(50% - 16.8px))"
                Style.backgroundSize "200% 100%"
                Style.mixBlendMode MixBlendMode.ColorDodge
            ])
            props.children
        }
