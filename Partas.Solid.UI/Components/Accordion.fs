...
[<AutoOpen>]
module AccordionStorybook =
    open Storybook.ComputationExpression
    [<Global>]
    let accordion = Accordion()
    [<SolidComponent>]
    let accordionStorybook = meta<Accordion> {
        component' Accordion
        argTypes {
            nameof accordion.multiple, argType {
                control ControlType.Boolean
                table {
                    defaultValue (tableInfo { summary "false" })
                }
                description "Whether to allow multiple open accordions"
            }
            nameof accordion.collapsible, argType {
                control ControlType.Boolean
                table {
                    defaultValue (tableInfo { summary "false" })
                }
                description "Whether you can close an open accordion"
            }
            nameof accordion.onChange, argType {
                control false
            }
        }
        args [
            nameof accordion.multiple, false
            nameof accordion.collapsible, false
        ]
        render (fun o ->
            Accordion().spread o {
                For(each = [|1;2;3;4;5|]) {
